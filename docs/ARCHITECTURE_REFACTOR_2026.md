...
基于重构前 HEAD `448adb1` 建立 `scripts/frozen-files.sha256`。

2026-08-02，项目维护者在确认进度逻辑理解后，授权在硬约束之外重构原冻结代码。冻结哈希已按授权后实现更新；官方进度初始化、`maxOffset`、`maxPages`、`progress`、`turningPages` 公式和所有经验参数保持不变。
...
冻结清单仍用来防止意外改动；明确授权的代码重构完成后必须先用回归测试证明硬约束，再更新哈希。不得借架构重构改变：
...
10. 不要顺手改冻结页面行为；需要改时独立评审、真站验收并更新哈希。
11. 静态检查、模拟 E2E 和真机验收要分别报告，不能互相替代。
12. 修改架构、命令、设置 schema、测试数量或冻结边界后同步更新文档。
