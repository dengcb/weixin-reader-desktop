...
安装后的桌面版也可以直接双击 `.atrd`。应用会先展示网站图标、插件 ID、版本、作者、接管域名、文件 SHA-256 和冲突信息，用户确认后才会安装。`weread` 与 `weread.qq.com` 属于内置微信读书，不能被外部插件占用；不同插件也不能声明相同或父子重叠的域名。相同外部插件 ID 只允许在明确确认后整体覆盖。
